...
html,body,[class*="css"],.stApp{font-family:'Inter',system-ui,sans-serif!important;background:#0b1017!important;color:#7a8fa6!important;-webkit-font-smoothing:antialiased!important}
#MainMenu,footer,header,.stDeployButton,[data-testid="collapsedControl"],[data-testid="stSidebar"],section[data-testid="stSidebar"]{display:none!important}
.main .block-container{max-width:1280px!important;margin:0 auto!important;padding:0 3rem 5rem!important}
...
.nav{height:56px;display:flex;align-items:center;justify-content:space-between;padding:0 3rem;background:#0b1017;border-bottom:1px solid rgba(255,255,255,0.06);position:sticky;top:0;z-index:999;margin-left:-3rem;margin-right:-3rem}
.nav-brand{display:flex;align-items:center;gap:11px}
.nav-mark{width:30px;height:30px;border-radius:7px;background:linear-gradient(135deg,#00c98d,#0096d6);display:flex;align-items:center;justify-content:center;font-size:.78rem;font-weight:700;color:#fff;flex-shrink:0;letter-spacing:.02em}
.nav-name{font-size:.88rem;font-weight:600;color:#dde6f0;letter-spacing:-.01em}
.nav-sub{font-size:.57rem;color:#00c98d;text-transform:uppercase;letter-spacing:.13em;font-weight:600;margin-top:1px}
.nav-pill{display:flex;align-items:center;gap:5px;font-size:.67rem;color:#354d62;background:#111c28;border:1px solid rgba(255,255,255,0.06);border-radius:5px;padding:3px 10px}
.live-dot{width:5px;height:5px;border-radius:50%;background:#00c98d;animation:bl 2.5s infinite}
@keyframes bl{0%,100%{opacity:1}50%{opacity:.2}}
[data-testid="stRadio"]{padding:0 3rem!important;margin:0 -3rem 0!important;background:#0b1017;border-bottom:1px solid rgba(255,255,255,0.06)}
[data-testid="stRadio"]>label{display:none!important}
[data-testid="stRadio"] [role="radiogroup"]{display:flex!important;gap:0!important;overflow-x:auto!important;scrollbar-width:none!important}
[data-testid="stRadio"] [role="radiogroup"]::-webkit-scrollbar{display:none!important}
[data-testid="stRadio"] [role="radiogroup"] label{display:flex!important;align-items:center!important;padding:13px 20px!important;font-size:.77rem!important;font-weight:500!important;color:#354d62!important;border:none!important;border-bottom:2px solid transparent!important;border-radius:0!important;background:transparent!important;white-space:nowrap!important;cursor:pointer!important;transition:color .15s,border-color .15s!important}
[data-testid="stRadio"] [role="radiogroup"] label>span:first-child,[data-testid="stRadio"] [role="radiogroup"] label>span:first-child *,[data-testid="stRadio"] input[type="radio"]{display:none!important}
[data-testid="stRadio"] [role="radiogroup"] label>div,[data-testid="stRadio"] [role="radiogroup"] label p{font-size:.77rem!important;font-weight:500!important;color:inherit!important;margin:0!important;padding:0!important}
[data-testid="stRadio"] [role="radiogroup"] label:has(input:checked){color:#00c98d!important;border-bottom-color:#00c98d!important;font-weight:600!important}
[data-testid="stRadio"] [role="radiogroup"] label:hover:not(:has(input:checked)){color:#5a7a94!important}
.ph{padding:32px 0 24px}
.ph-eye{font-size:.59rem;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:#00c98d;margin-bottom:6px}
.ph-title{font-size:1.5rem;font-weight:700;color:#dde6f0;letter-spacing:-.025em;line-height:1.2;margin-bottom:5px}
.ph-title em{font-style:normal;color:#00c98d}
.ph-sub{font-size:.8rem;color:#2d4255;font-weight:400;line-height:1.55}
.krow{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:28px}
.kcard{background:#111c28;border:1px solid rgba(255,255,255,0.06);border-radius:10px;padding:20px 22px 18px;position:relative;overflow:hidden}
.kcard::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:var(--a,linear-gradient(90deg,#00c98d,#0096d6))}
.kcard-val{font-size:1.6rem;font-weight:700;color:#dde6f0;letter-spacing:-.04em;line-height:1;margin-bottom:5px;margin-top:2px}
.kcard-lbl{font-size:.62rem;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#1e3247}
.kcard-delta{font-size:.68rem;color:#00c98d;margin-top:3px;font-weight:600}
.srow{display:grid;grid-template-columns:repeat(4,1fr);gap:1px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.06);border-radius:10px;overflow:hidden;margin-bottom:20px}
.srow-item{background:#111c28;padding:16px 18px}
.srow-val{font-size:1.15rem;font-weight:700;color:#dde6f0;letter-spacing:-.03em}
.srow-lbl{font-size:.58rem;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#1e3247;margin-top:3px}
.sec{font-size:.6rem;font-weight:700;text-transform:uppercase;letter-spacing:.13em;color:#1e3247;margin:24px 0 10px;display:flex;align-items:center;gap:10px}
.sec::after{content:'';flex:1;height:1px;background:rgba(255,255,255,0.05)}
label,[data-testid="stWidgetLabel"]{color:#1e3247!important;font-size:.6rem!important;font-weight:700!important;text-transform:uppercase!important;letter-spacing:.1em!important;margin-bottom:4px!important}
.stSelectbox>div>div{background:#111c28!important;border:1px solid rgba(255,255,255,0.07)!important;border-radius:7px!important;color:#9eb8cc!important;font-size:.81rem!important;min-height:36px!important}
.stSelectbox>div>div:focus-within{border-color:rgba(0,201,141,0.4)!important;box-shadow:0 0 0 2px rgba(0,201,141,0.07)!important}
.stSelectbox svg{fill:#2d4255!important}
.stTextInput>div>div>input{background:#111c28!important;border:1px solid rgba(255,255,255,0.07)!important;border-radius:7px!important;color:#9eb8cc!important;font-size:.81rem!important;height:36px!important;padding:0 12px!important}
.stTextInput>div>div>input:focus{border-color:rgba(0,201,141,0.4)!important;box-shadow:0 0 0 2px rgba(0,201,141,0.07)!important;outline:none!important}
...
[data-baseweb="slider"] div[role="slider"]{background:#00c98d!important;border:2px solid #0b1017!important;width:14px!important;height:14px!important;box-shadow:0 0 0 2px rgba(0,201,141,0.3)!important}
[data-testid="stTickBar"]{display:none!important}
[data-testid="stMetric"]{padding:0!important}
[data-testid="stMetricValue"]{font-size:1.3rem!important;font-weight:700!important;color:#dde6f0!important;letter-spacing:-.03em!important;line-height:1.1!important}
[data-testid="stMetricLabel"]{color:#1e3247!important;font-size:.6rem!important;text-transform:uppercase!important;letter-spacing:.1em!important;font-weight:700!important}
.stDataFrame{border-radius:8px!important;border:1px solid rgba(255,255,255,0.06)!important;overflow:hidden!important}
.stDownloadButton>button{background:rgba(0,201,141,0.07)!important;border:1px solid rgba(0,201,141,0.2)!important;color:#00c98d!important;border-radius:6px!important;font-size:.73rem!important;font-weight:600!important;padding:5px 14px!important;height:30px!important}
.src{background:#111c28;border:1px solid rgba(255,255,255,0.06);border-radius:10px;padding:18px 10px;text-align:center;height:100%}
.ibox{background:rgba(0,201,141,0.05);border-left:2px solid #00c98d;border-radius:6px;padding:12px 16px;font-size:.79rem;color:#3a9e74;line-height:1.6}
.rgpd{background:#111c28;border:1px solid rgba(255,255,255,0.06);border-radius:8px;padding:16px 18px;margin-top:12px;font-size:.74rem;color:#354d62;line-height:1.85}
.rgpd-h{font-size:.6rem;font-weight:700;color:#00c98d;text-transform:uppercase;letter-spacing:.1em;margin-bottom:8px}
...
_AX  = dict(gridcolor='rgba(255,255,255,0.04)', linecolor='rgba(255,255,255,0.06)',
...
def load_data():
    ...
    try:
        ...
        query = """
            SELECT
                ...
                t.emission_co2_gkm AS emissions_co2_gkm,
                t.source_donnee,
                t.created_at
            FROM train t
        ...
        if df is None:
            try:
                engine = create_engine(DB_URL)
                df = pd.read_sql("SELECT * FROM dessertes", engine)
    ...
    # Pour la compatibilite avec les pages : origin_station = gare
    if 'destination_station' not in df.columns:
        df['destination_station'] = df['origin_station']
    return df
...
def page_accueil(df):
    ...
    kpis([
        (f"{len(df):,}",   "Trains total",     "linear-gradient(90deg,#00c98d,#0096d6)", None),
    ...
    with c1:
        ...
        if 'type_service' in df.columns:
            ...
            fig = px.bar(g, x='operator', y='n', color='type_service', barmode='group',
                         ...
                         labels={'n':'Trains','operator':'','type_service':''})
...
def page_horaires(df):
    ph("Recherche", "<em>Horaires</em> & trains",
       "Filtrez par gare, operateur ou distance")

    gares = ["Toutes"] + sorted(df['origin_station'].dropna().unique().tolist())

    c1,c2,c3,c4 = st.columns(4)
    with c1: op  = st.selectbox("Operateur", ["Tous"]+sorted(df['operator'].dropna().unique()))
    with c2: ts  = st.selectbox("Type service", ["Tous","Jour","Nuit"]) if 'type_service' in df.columns else "Tous"
    with c3: dep = st.selectbox("Gare", gares)
    with c4: tl  = st.selectbox("Type ligne", ["Tous","national","regional"]) if 'type_ligne' in df.columns else "Tous"
    dist = st.slider("Distance (km)", 0, 917, (0,917), step=50)
    ...
    if tl  != "Tous" and 'type_ligne' in f.columns: f = f[f['type_ligne']==tl]
    ...
    if f.empty:
        st.markdown('<div class="ibox">Aucun train trouve.</div>', unsafe_allow_html=True)
        return

    dcols = ['operator','origin_station']
    if 'heure_depart' in f.columns: dcols += ['heure_depart','heure_arrivee']
    dcols += ['distance_km','co2_emission_kg','type_service']
    ds = f[[c for c in dcols if c in f.columns]].head(500).copy()
    ds['distance_km']     = ds['distance_km'].round(1)
    ds['co2_emission_kg'] = ds['co2_emission_kg'].round(3)
    ds = ds.rename(columns={
        'operator':'Operateur','origin_station':'Gare',
        'heure_depart':'H. depart','heure_arrivee':'H. arrivee',
        'distance_km':'Dist. km','co2_emission_kg':'CO2 kg','type_service':'Type'})
    st.dataframe(ds, use_container_width=True, height=360)

    cl,_ = st.columns([1,6])
    with cl: st.download_button("Export CSV", ds.to_csv(index=False).encode(), "trains.csv","text/csv")
...
def page_statistiques(df):
    ph("Analyse", "<em>Statistiques</em> reseau",
       "Couverture ferroviaire integree par operateur")
    ...
    kpis([
        ...
        (f"{nb_n:,}",       "Trains de nuit",  "linear-gradient(90deg,#6366f1,#818cf8)", None),
        (f"{nb_j:,}",       "Trains de jour",  "linear-gradient(90deg,#f59e0b,#fbbf24)", None),
        (f"{df['distance_km'].mean():.0f} km", "Distance moy.", "linear-gradient(90deg,#0096d6,#38bdf8)", None),
        (f"{df['distance_km'].max():.0f} km",  "Distance max.", "linear-gradient(90deg,#354d62,#4a6275)", None),
    ])
    ...
    with c2:
        ...
        st.plotly_chart(chart(fig, h=360), use_container_width=True, config=CFG)
    ...
    with c3:
        if 'type_service' in df.columns:
            ...
            st.plotly_chart(fig, use_container_width=True, config=CFG)
    with c4:
        sec("Trains par operateur")
        op = df['operator'].value_counts().reset_index(); op.columns=['Op','Nb']
        fig = px.bar(op, x='Op', y='Nb', color='Nb',
    ...
    grp = df.groupby('operator').agg(
        Trains=('operator','count'), Dist_moy=('distance_km','mean'),
...
def page_liaisons(df):
    ph("Flux", "Directions & <em>Liaisons</em>",
       "Analyse des flux ferroviaires par operateur et type")
    ...
    cl,cr = st.columns(2)
    with cl:
        sec(f"Top {top_n} gares les plus desservies")
        top = f['origin_station'].value_counts().head(top_n).reset_index(); top.columns=['Gare','Nb']
        fig = px.bar(top, x='Nb', y='Gare', orientation='h', color='Nb',
                     color_continuous_scale=[[0,'#0a2818'],[1,'#00c98d']])
        fig.update_traces(marker_line_width=0)
...
def page_co2(df):
    ...
    co2_train_pkm = 14
    co2_avion_pkm = 258
    tc  = td * co2_train_pkm / 1000
    ca  = td * co2_avion_pkm / 1000
    ev  = ca - tc
    pct = ev / ca * 100 if ca > 0 else 0
    ratio = round(co2_avion_pkm / co2_train_pkm)

    kpis([
        (f"{tc:,.0f} kg", "CO2 si tout en train",  "linear-gradient(90deg,#00c98d,#34d399)", f"14 g/passager-km (ADEME 2023)"),
        (f"{ca:,.0f} kg", "CO2 si tout en avion",  "linear-gradient(90deg,#ef4444,#f87171)", f"258 g/passager-km"),
        (f"{ev:,.0f} kg", "CO2 evite",             "linear-gradient(90deg,#0096d6,#38bdf8)", f"- {pct:.0f}% vs avion"),
        (f"x {ratio}",    "Avion plus emetteur",   "linear-gradient(90deg,#f59e0b,#fbbf24)", f"{co2_avion_pkm} / {co2_train_pkm} g/km"),
    ])

    sec("Comparatif modal (g CO2 / passager-km)")
    comp = pd.DataFrame({
        'Mode':['Train electrique','Train moy. UE','Avion long-courrier','Avion court-courrier'],
        'gkm':[6,14,195,258],'color':['#00c98d','#0096d6','#f87171','#ef4444']})
    ...
    c1,c2 = st.columns(2)
    with c1:
        sec("Distribution des emissions par train")
    ...
    if 'type_service' in df.columns:
        st.markdown("<br>", unsafe_allow_html=True)
        sec("Comparatif Jour vs Nuit")
        tbl = df.groupby('type_service').agg(
            Nb=('co2_emission_kg','count'), CO2_total=('co2_emission_kg','sum'),
            CO2_moy=('co2_emission_kg','mean'), Dist_moy=('distance_km','mean')
        ).round(3).reset_index()
        tbl.columns = ['Type','Nb trains','CO2 total (kg)','CO2 moy (kg)','Distance moy (km)']
...
def page_qualite(df, stats):
    ...
    sc = 'source_donnee' if 'source_donnee' in df.columns else None
    if sc:
        st.markdown("<br>", unsafe_allow_html=True)
